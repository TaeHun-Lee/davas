...
### 현재 비공식 운영 단계의 법률 문서 기준

Davas를 개인 Raspberry Pi에서 비공식·비상업 self-hosting으로 운영하는 동안 승인된 이용약관·개인정보처리방침 원문은 release 필수 조건이 아니다. 개발용 legal route와 consent version은 향후 전환 호환성을 위해 유지할 수 있다.
...
| LEGAL-01    | 약관·개인정보  | 현재 동의 문서 읽기          | 이전 화면                  |
...
**목적** 가입 전에 현재 동의 문서와 version을 읽을 수 있게 한다.

- 두 route는 로그인 없이 열 수 있고 `BackHeader`를 사용한다.
- 현재 비공식·비상업 self-hosting에서는 개발용 fixture임을 화면에 분명히 표시하고 사용할 수 있으며, release 과정에서 legal 원문을 검사하지 않는다.
- 법률 문구를 Codex가 임의로 공식 문서처럼 작성하지 않는다. 운영자가 승인 원문을 제공하는 별도 제품 변경을 진행할 때 versioned 원문은 `apps/web/src/content/legal` 같은 단일 위치에서 관리하고 화면은 그 원문만 렌더링한다.
