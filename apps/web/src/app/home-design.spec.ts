...
describe('Davas authenticated home design', () => {
  ...
  it('keeps the top header and bottom tab bar as shared layout components', () => {
    ...
    assert.match(headerSource, /isDrawerOpen/);
    assert.match(headerSource, /setIsDrawerOpen\(true\)/);
    assert.match(headerSource, /data-design="left-drawer-overlay"/);
    assert.match(headerSource, /data-design="left-drawer-panel"/);
    assert.match(headerSource, /aria-label="메뉴 닫기"/);
    assert.match(headerSource, /translate-x-0/);
    assert.match(headerSource, /-translate-x-full/);
    assert.match(headerSource, /href: '\/profile'/);
    assert.match(headerSource, /href: '\/diary'/);
    assert.match(headerSource, /data-design="profile-avatar"/);
    assert.match(headerSource, /getMe/);
    assert.match(headerSource, /normalizeProfileImageUrl/);
    assert.match(headerSource, /profileImageUrl/);
    assert.match(headerSource, /<img/);
    assert.match(headerSource, /href="\/profile"/);
    assert.match(headerSource, /프로필 화면으로 이동/);
    assert.doesNotMatch(headerSource, /<button type="button" aria-label="프로필 보기"/);
