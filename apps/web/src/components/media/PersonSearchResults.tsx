  ...
  return (
    <section className="mt-5">
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-[16px] font-extrabold leading-[22px] tracking-[-0.02em] text-[#1f2a44]">배우 검색 결과</h2>
        <span className="text-[11px] font-bold text-[#9aa6b8]">이름으로 출연작 찾기</span>
      </div>
      {status === 'searching' ? <div className="card-surface mt-3 rounded-[18px] p-4 text-[13px] font-bold text-[#8b96a8]">배우를 검색 중이에요...</div> : null}
      {status === 'empty' ? <div className="card-surface mt-3 rounded-[18px] p-4 text-[13px] font-bold text-[#8b96a8]">일치하는 배우가 없어요</div> : null}
      {status === 'error' ? <div className="card-surface mt-3 rounded-[18px] p-4 text-[13px] font-bold text-[#ef4444]">배우 검색을 불러오지 못했어요.</div> : null}
      {status === 'results' ? (
          ...
          {items.map((person) => {
            ...
            return (
              ...
              >
                ...
                <span className="shrink-0 rounded-full bg-[#edf5ff] px-3 py-2 text-[11px] font-extrabold text-[#216bd8]">출연작 보기</span>
              </button>
            );
          })}
        </div>
