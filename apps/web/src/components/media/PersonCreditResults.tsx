  ...
  if (item.posterUrl) {
    return (
      <img
        src={item.posterUrl}
        alt={`${item.title} 포스터`}
        className="h-[86px] w-[58px] shrink-0 rounded-[13px] object-cover shadow-[0_8px_18px_rgba(21,38,69,0.13)]"
      />
    );
  }
...
function formatCreditMeta(item: MediaSearchResult) {
  ...
  const genres = getTmdbGenreNames({ genreIds: item.genreIds, mediaType: item.mediaType })
    .slice(0, 2)
    .join(' · ');
  ...
  return (
    <section className="mt-5">
      <div className="flex items-center justify-between gap-3">
        ...
        {personName ? (
          <span className="truncate text-[11px] font-bold text-[#9aa6b8]">{personName}</span>
        ) : null}
      </div>
      {status === 'loading' ? (
        <MediaDetailLoadingIndicator label="출연 작품을 불러오는 중" fullScreen={false} />
      ) : null}
      {status === 'empty' ? (
          ...
          {items.map((item) => (
            ...
            >
              ...
              <span className="flex min-w-0 flex-1 flex-col py-1">
                <span className="line-clamp-1 text-[15px] font-extrabold leading-[20px] text-[#1f2a44]">
                  {item.title}
                </span>
                <span className="mt-1 text-[11px] font-bold leading-[15px] text-[#8b96a8]">
                  {formatCreditMeta(item)}
                </span>
                <span className="mt-2 line-clamp-2 text-[11px] font-semibold leading-[16px] text-[#747f91]">
                  {item.overview || 'TMDB에서 제공한 소개가 아직 없어요.'}
                </span>
                <span className="mt-auto pt-2 text-[11px] font-extrabold text-[#216bd8]">
                  작품 선택
                </span>
              </span>
            </button>
