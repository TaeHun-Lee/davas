  ...
  if (item.posterUrl) {
    return (
      <img
        src={item.posterUrl}
        alt={`${item.title} 포스터`}
        className="h-[92px] w-[62px] shrink-0 rounded-[12px] object-cover shadow-[0_8px_16px_rgba(21,38,69,0.12)]"
      />
    );
  }

  return (
    <div className="h-[92px] w-[62px] shrink-0 rounded-[12px] bg-gradient-to-br from-[#dbe7f8] to-[#9fb9df] shadow-[0_8px_16px_rgba(21,38,69,0.12)]" />
  );
}
...
export function MediaSearchResults({
  ...
  return (
    <section className="mt-5">
      ...
      {status === 'searching' ? (
        <MediaDetailLoadingIndicator label="검색 중" fullScreen={false} />
      ) : null}
      {status === 'empty' ? (
      ...
      {status === 'error' ? (
        <div className="card-surface mt-3 rounded-[18px] p-4 text-[13px] font-bold text-[#ef4444]">
          검색을 불러오지 못했어요. TMDB API 설정을 확인해주세요.
        </div>
      ) : null}
      {status === 'results' ? (
        <div className="mt-3 space-y-3">
          {items.map((item) => {
            ...
            const genreNames = getTmdbGenreNames({
              genreIds: item.genreIds,
              mediaType: item.mediaType,
            });
            const genreText = genreNames.slice(0, 2).join(' · ');
            const mediaMeta = [item.mediaType === 'TV' ? '드라마' : '영화', releaseYear, genreText]
              .filter(Boolean)
              .join(' · ');
            return (
              <button
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-2 block line-clamp-2 text-[11px] font-semibold leading-[17px] text-[#788395]">
                    {item.overview || '작품 소개가 아직 준비되지 않았어요.'}
                  </span>
                </span>
              </button>
            );
