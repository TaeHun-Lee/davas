...
function HeroImage({ item }: { item: MediaHeroCarouselItem }) {
  if (item.imageVariant === 'poster') {
    return (
      <div
        data-design="archive-poster"
        className="relative h-[154px] w-[126px] shrink-0 overflow-hidden rounded-[14px] shadow-[0_12px_20px_rgba(22,43,75,0.18)] max-[374px]:h-[142px] max-[374px]:w-[112px]"
      >
        {item.imageUrl ? (
          <img
            src={item.imageUrl}
            alt={item.imageAlt ?? `${item.title} 포스터`}
            className="h-full w-full object-cover object-center"
          />
        ) : (
          <div className="h-full w-full rounded-[14px] bg-gradient-to-br from-[#e9eef7] via-[#f6f8fc] to-[#dfe8f5]" />
        )}
      </div>
    );
  }
  ...
  return (
    <div
      data-design={item.imageUrl ? 'recommendation-still' : 'recommendation-still-placeholder'}
      className="relative h-full min-h-[168px] overflow-hidden rounded-[18px] bg-gradient-to-br from-[#0b1630] via-[#1e4f82] to-[#d99a66] shadow-[0_14px_28px_rgba(20,45,83,0.16)]"
    >
      {item.imageUrl ? (
        <img
          src={item.imageUrl}
...
export function MediaHeroCarousel({
  ...
  return (
    <section
      data-design="media-hero-carousel"
      className={`today-recommendation-card archive-gradient-card card-surface relative rounded-[24px] p-4 shadow-[0_14px_32px_rgba(31,65,114,0.09)] ${className}`}
    >
      {cardLabel ? (
      ...
      <div
        className="today-carousel-viewport overflow-hidden rounded-[20px]"
      ...
      >
        {carouselItems.length === 0 ? (
          ...
          <div
            className="today-carousel-track flex transition-transform duration-500 ease-out"
            ...
            {carouselItems.map((item) => {
              ...
              return (
                <div key={item.id} className="w-full shrink-0">
                  <div
                    className={
                      item.imageVariant === 'poster'
                        ? 'flex gap-4 max-[374px]:gap-3'
                        : 'grid grid-cols-2 items-stretch gap-3 max-[430px]:grid-cols-1 max-[430px]:gap-4'
                    }
                  >
                    <HeroImage item={item} />
                    <div
                      className={
                        item.imageVariant === 'poster'
                          ...
                          : 'flex min-w-0 flex-col py-1 max-[430px]:py-0'
                      }
                    >
                      <div>
                        {item.eyebrow ? (
                          <span className="inline-flex rounded-full bg-[#eef5ff] px-3 py-1 text-[11px] font-extrabold text-[#2f7eea]">
                            {item.eyebrow}
                          </span>
                        ) : null}
                        <h1
                          className={
                            item.imageVariant === 'poster'
                              ? 'mt-2.5 text-[25px] font-black leading-[29px] tracking-[-0.04em] text-[#132b55] max-[374px]:text-[22px]'
                              : 'line-clamp-2 text-[20px] font-black leading-[25px] tracking-[-0.035em] text-[#172947]'
                          }
                        >
                          {item.title}
                        </h1>
                        {item.meta ? (
                          <p className="mt-1.5 text-[12px] font-bold leading-[16px] text-[#8b96a8]">
                            {item.meta}
                          </p>
                        ) : null}
                        {item.description ? (
                          <p className="mt-3 line-clamp-3 text-[12px] font-semibold leading-[18px] text-[#747f91]">
                            {item.description}
                          </p>
                        ) : null}
                      </div>
                      {itemActions.length > 0 ? (
                        <div
                          className={
                            item.imageVariant === 'poster'
                              ? 'archive-action-row mt-auto flex items-center gap-2 pt-3.5'
                              : 'today-recommendation-actions mt-3 grid grid-cols-1 gap-0 pt-2 pr-2 pb-2'
                          }
                        >
                          {itemActions.map((action) => (
                            <button
                              key={action.label}
                              data-design={action.dataDesign}
                              type="button"
        ...
        <div className="carousel-indicator mt-2 flex justify-center gap-[5px]">
          {carouselItems.map((item, index) => (
            <button
              ...
              className={
                index === activeIndex
                  ? 'h-[5px] w-[22px] rounded-full bg-[#2f7eea] transition-all duration-300'
                  : 'h-[5px] w-[5px] rounded-full bg-[#dbe5f3] transition-all duration-300'
              }
            />
          ))}
        </div>
      ...
      {autoPlay && carouselItems.length > 1 ? (
        <button
          ...
          className="mx-auto mt-2 flex min-h-11 items-center rounded-full px-4 text-[11px] font-extrabold text-[#526078]"
        >
          {isPaused ? '자동 넘김 시작' : '자동 넘김 정지'}
        </button>
      ) : null}
    </section>
  );
