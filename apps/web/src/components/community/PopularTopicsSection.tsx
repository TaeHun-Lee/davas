...
export function PopularTopicsSection({ topics }: PopularTopicsSectionProps) {
  ...
  return (
    <section className="mb-6" aria-labelledby="popular-topics-title">
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="text-[18px] font-extrabold text-[#9aa6b8]" aria-hidden="true">›</span>
      </div>
      <div className="-mx-4 overflow-x-auto px-4 min-[390px]:-mx-5 min-[390px]:px-5">
        <div className="flex w-max gap-2 pb-1">
          {topics.map((topic) => (
            <span key={topic.label} className="rounded-full border border-[#e2e8f2] bg-white px-4 py-2 text-[12px] font-extrabold text-[#4c5b73] shadow-[0_6px_16px_rgba(31,65,114,0.05)]">
              {topic.label}
            </span>
          ))}
        </div>
