...
import {
  mapMediaDetailToDiaryMedia,
  todayIsoDate,
  validateDiaryCompose,
} from './diary-compose-utils';
...
export function DiaryComposeScreen({
  ...
  useEffect(() => {
    getFriends()
      .then((result) => setFriendOptions(result.friends.map((item) => item.user)))
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setSubmitError(
        mode === 'edit'
          ? '다이어리를 수정하지 못했어요. 잠시 후 다시 시도해주세요.'
          : '다이어리를 저장하지 못했어요. 잠시 후 다시 시도해주세요.',
      );
    } finally {
  ...
  return (
    <main className="flex min-h-screen justify-center overflow-x-hidden bg-[#172947]/35 text-[#1f2a44] backdrop-blur-sm">
      <section
        data-design="diary-compose-shell"
        className="min-h-dvh w-full max-w-[430px] overflow-x-hidden bg-[#f8fafd] px-5 pb-28 shadow-[0_0_40px_rgba(15,23,42,0.18)]"
      >
        <DiaryComposeHeader onBack={handleBack} />
        <div className="mx-auto flex w-full max-w-[430px] flex-col gap-4 pt-4">
          {mediaStatus === 'loading' && Boolean(mediaId || diaryId) ? (
            <MediaDetailLoadingIndicator />
          ) : null}
          {mediaStatus === 'idle' ? (
            <section className="card-surface rounded-[24px] p-6 text-center">
              <h1 className="text-[18px] font-black text-[#23426f]">먼저 작품을 선택해주세요</h1>
              <p className="mt-2 text-[13px] font-bold text-[#65758a]">
                작품이 없는 기록은 저장할 수 없어요.
              </p>
              <button
                type="button"
          ...
          {mediaStatus === 'error' ? (
            <section className="rounded-[20px] bg-white px-4 py-4 text-center shadow-[0_12px_28px_rgba(31,65,114,0.08)]">
              <p className="text-[13px] font-bold text-[#d9413a]">
                작품 정보를 불러오지 못했어요. 이 상태에서는 저장할 수 없습니다.
              </p>
              <div className="mt-3 flex justify-center gap-2">
                <button
                  type="button"
                  onClick={() => setRetryKey((value) => value + 1)}
                  className="min-h-11 rounded-[14px] bg-[#284778] px-4 text-[12px] font-black text-white"
                >
                  다시 시도
                </button>
                <button
                  type="button"
                  onClick={() => router.push('/explore?intent=record')}
                  className="min-h-11 rounded-[14px] bg-[#eef3f8] px-4 text-[12px] font-black text-[#284778]"
                >
                  다시 선택
                </button>
              </div>
          ...
          {mediaStatus !== 'idle' ? (
            <SelectedMediaCard
              media={selectedMedia}
              isLoading={mediaStatus === 'loading' && Boolean(mediaId || diaryId)}
            />
          ) : null}
          {mediaStatus === 'ready' ? (
            <>
              ...
              <DiaryTitleField
                value={title}
                fallbackTitle={selectedMedia?.title ?? ''}
                onChange={setTitle}
              />
              <DiaryContentField value={content} onChange={setContent} />
              <TogetherMomentSection
                companions={companions}
                onChangeCompanions={setCompanions}
                friendOptions={friendOptions}
                watchedPlace={watchedPlace}
                onChangeWatchedPlace={setWatchedPlace}
                mood={mood}
                onChangeMood={setMood}
                memoryNote={memoryNote}
                onChangeMemoryNote={setMemoryNote}
              />
              {visibility === 'SELECTED' ? (
                <fieldset className="rounded-[22px] bg-white p-4 shadow-[0_10px_22px_rgba(31,65,114,0.06)]">
                  <legend className="px-1 text-[14px] font-black text-[#23426f]">
                    공개할 친구 선택
                  </legend>
                  <p className="mt-1 text-[12px] font-bold text-[#65758a]">
                    선택한 친구만 기록과 함께 본 정보를 볼 수 있어요.
                  </p>
                  {friendOptions.length ? (
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      {friendOptions.map((friend) => (
                        <label
                          key={friend.id}
                          className="flex min-h-11 items-center gap-2 rounded-[14px] bg-[#f3f7fc] px-3 text-[12px] font-black text-[#284778]"
                        >
                          <input
                            type="checkbox"
                    ...
                    <p className="mt-3 rounded-[14px] bg-[#fff1f0] p-3 text-[12px] font-bold text-[#a93530]">
                      먼저 친구 요청을 수락한 친구가 필요해요.
                    </p>
                  )}
                </fieldset>
              ) : null}
