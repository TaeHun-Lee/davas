          ...
          {items.map((item) => {
            const card = (
              <>
                ...
                />
                <h3 className="mt-2 line-clamp-2 text-[11px] font-extrabold leading-4 text-[#27334a]">
                  {item.title}
                </h3>
                <p className="mt-0.5 truncate text-[10px] font-semibold text-[#99a4b5]">
                  {item.meta}
                </p>
              ...
              <article key={`${item.title}-${item.meta}`} className={`${itemClassName} shrink-0`}>
                {onItemClick ? (
                  ...
                  <button
                    ...
                    aria-label={`${item.title} 상세 보기`}
                    className="block w-full text-left"
                    onClick={() => onSelect(item.sourceItem!)}
                  >
                    {card}
                  </button>
                ) : (
