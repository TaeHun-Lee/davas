...
function NotebookIcon() {
  return (
    ...
    >
      ...
      <path
        d="M10.1 4.2h8a2.1 2.1 0 0 1 2.1 2.1v13.2a2.1 2.1 0 0 1-2.1 2.1h-8V4.2Z"
        fill="#DDEBFF"
      />
      ...
      <path
        d="M12.2 15.3h5.3M12.2 17.5h3.9"
        stroke="#2F7EEA"
        strokeWidth="1.35"
        strokeLinecap="round"
      />
      <path
        d="M8.4 21.3c1.1.8 7.4.9 10.2.2"
        stroke="#BFD4F8"
        strokeWidth="1.4"
        strokeLinecap="round"
      />
...
function CalendarWatchIcon() {
  return (
    <svg
      data-design="stat-calendar-icon"
      width="28"
      height="28"
      viewBox="0 0 28 28"
      fill="none"
...
function MasksIcon() {
  return (
    ...
    >
      <path
        d="M5.3 7.6c4.2-1.8 7.8-1.8 11.5 0-.1 6.7-2 10.9-5.8 12.1-3.8-1.2-5.6-5.4-5.7-12.1Z"
        fill="#2F7EEA"
      />
      <path
        d="M12 8.2c3.7-1.4 7.4-1.1 10.8.9-.5 6.2-2.7 10.1-6.4 11-2-.7-3.5-2.3-4.4-4.8V8.2Z"
        fill="#EF4444"
      />
      <circle cx="9.2" cy="11.8" r="1" fill="white" />
      <circle cx="13.1" cy="11.8" r="1" fill="white" />
      <path
        d="M8.5 15.9c1.3 1 3.5 1 4.8 0"
        stroke="white"
        strokeWidth="1.35"
        strokeLinecap="round"
      />
      ...
      <path
        d="M15.7 16.5c1.1-1 3.1-1 4.1 0"
        stroke="white"
        strokeWidth="1.25"
        strokeLinecap="round"
      />
...
function StatIcon({ kind }: { kind: StatKind }) {
  const circleTone =
    kind === 'rating' || kind === 'genre'
      ? 'stat-icon-circle-red bg-[#fff2f2]'
      : 'stat-icon-circle-blue bg-[#f1f6ff]';

  return (
    <span
      className={`stat-icon-circle flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-full ${circleTone}`}
    >
  ...
  return (
    <section className="stats-grid mt-4 grid grid-cols-2 gap-[10px]">
      {stats.map((stat) => (
        <article
          key={stat.label}
          className="stat-card flex min-h-[76px] items-center gap-3 rounded-[18px] bg-white px-3.5 py-3 shadow-[0_5px_14px_rgba(21,38,69,0.07)] ring-1 ring-[#edf2f8] max-[374px]:gap-2.5 max-[374px]:px-3"
        >
          <StatIcon kind={stat.kind} />
          <div className="min-w-0 flex-1">
            <p className="truncate text-[12px] font-bold text-[#243047] leading-[14px]">
              {stat.label}
            </p>
            <p className="mt-[4px] whitespace-nowrap text-[21px] font-extrabold leading-[24px] tracking-[-0.03em] text-[#111827] max-[374px]:text-[20px]">
              {stat.value}
              {stat.unit ? (
                <span className="ml-1 align-baseline text-[12px] font-semibold tracking-[-0.015em] text-[#27334a]">
                  {stat.unit}
                </span>
              ) : null}
            </p>
            <p className="mt-[4px] truncate text-[10px] font-semibold leading-[12px] text-[#9aa4b2]">
              {stat.helper}
            </p>
          </div>
        </article>
