...
function buildCalendarDays(year: number, month: number) {
  ...
  const days = Array.from({ length: totalSlots }, (_, index) => {
    if (index < leadingDays) {
      return previousMonthDays - leadingDays + index + 1;
    }
    if (index < leadingDays + currentMonthDays) {
...
export function buildHomeDashboardView(dashboard: DiaryDashboardView): HomeDashboardView {
  ...
  const topRatedItems = [...recentItems]
    .sort(
      (a, b) =>
        b.rating - a.rating || Number(new Date(b.createdAt)) - Number(new Date(a.createdAt)),
    )
    .slice(0, 10);
  ...
  return {
    archiveHighlight: latestItem
      ? {
          ...
          meta: getPrimaryGenre(latestItem),
      ...
      : {
          ...
          quote: '작품을 찾고 감상을 기록해보세요.',
        },
    stats: [
      {
        label: '전체 다이어리',
        value: String(dashboard.summary.totalCount),
        unit: '개',
        helper: '모든 기록의 합계',
        kind: 'diary',
      },
      {
        label: '이번 달 관람 수',
        value: String(dashboard.summary.monthlyCount),
        unit: '편',
        helper: '이번 달 기록한 작품 수',
        kind: 'watch',
      },
      {
        label: '평균 평점',
        value: `${dashboard.summary.averageRating.toFixed(1)} / 5.0`,
        helper: '지금까지의 평균 평점',
        kind: 'rating',
      },
      {
        label: '최다 장르',
        value: dashboard.summary.topGenre?.name ?? '-',
        helper: '가장 많이 기록한 장르',
        kind: 'genre',
      },
    ],
    ...
    <>
      <AppShell nickname={user?.nickname}>
        <SearchEntry
          ...
          ariaLabel="기록할 영화나 드라마 검색하기"
          className="text-[13px] font-semibold leading-[18px] text-[#9aa6b8]"
        />
        <ArchiveHighlightSection
          item={view.archiveHighlight}
          onDetailSelect={openHomeMediaDetail}
        />
        <HomeStatsGrid stats={view.stats} />
        {view.favorites.length > 0 ? (
          <FavoriteMoviesSection movies={view.favorites} onDetailSelect={openHomeMediaDetail} />
        ) : null}
        <MonthlyWatchCalendarSection {...view.calendar} />
        {view.recentRecords.length > 0 ? (
          <RecentRecordsSection records={view.recentRecords} />
        ) : null}
      </AppShell>
      {selectedMedia ? (
        <MediaDetailModal
