    ...
    <section className="card-surface mt-6 rounded-[22px] p-4 max-[374px]:p-3.5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <div className="mt-2 flex gap-3 text-[10px] font-bold text-[#8b96a8]">
            <span>
              <b className="text-[#216bd8]">●</b> 관람한 날
            </span>
            <span>
              <b className="text-[#ff5050]">●</b> 기록한 다이어리
            </span>
          </div>
        </div>
        ...
        {days.map((day, index) => {
          ...
          return (
            <div key={`${day}-${index}`} className="relative flex h-7 items-center justify-center">
              <span
                className={`flex h-7 w-7 items-center justify-center rounded-full ${selected ? 'bg-[#216bd8] text-white' : isWatched ? 'bg-[#eaf1ff] text-[#216bd8]' : currentMonth ? '' : 'text-[#c1cad7]'}`}
              >
                {day}
              </span>
              {hasDiary ? (
                <span className="absolute bottom-0 h-1 w-1 rounded-full bg-[#ff5050]" />
              ) : null}
            </div>
          );
        })}
