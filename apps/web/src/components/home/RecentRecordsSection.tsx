  ...
  return (
    <>
      ...
      <section className="space-y-3">
        {records.map((record) => (
          <article
            key={record.diaryId ?? record.title}
            className="card-surface rounded-[18px] max-[374px]:gap-2.5"
          >
            <button
              type="button"
              aria-label={`${record.title} 다이어리 상세 보기`}
              className="flex min-h-11 w-full items-center gap-3 p-3 text-left max-[374px]:gap-2.5"
              onClick={() => openRecord(record)}
            >
              <MoviePosterVisual
                gradient={record.gradient ?? 'from-[#e9eef7] via-[#f6f8fc] to-[#dfe8f5]'}
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 truncate text-[11px] font-semibold text-[#8b96a8]">
                  {record.desc}
                </p>
              </div>
