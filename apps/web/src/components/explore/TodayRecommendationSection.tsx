                ...
                <div key={`${entry.externalId ?? entry.title}-${index}`} className="w-full shrink-0">
                  <div className="grid grid-cols-2 items-stretch gap-3 max-[430px]:grid-cols-1 max-[430px]:gap-4">
                    ...
                    <div className="flex min-w-0 flex-col py-1 max-[430px]:py-0">
                      ...
                      <div className="today-recommendation-actions mt-4 grid grid-cols-[0.9fr_1.1fr] gap-2.5 pt-3 pr-1.5 pb-3 max-[430px]:grid-cols-2 max-[430px]:pt-1">
                        <button data-design="today-detail-button" type="button" onClick={() => void onSelect?.(entry)} className="today-detail-button flex h-[40px] min-w-0 items-center justify-center whitespace-nowrap rounded-full border border-[#e8eef6] bg-white px-3 text-[12px] font-extrabold leading-[16px] text-[#536179] shadow-[0_5px_12px_rgba(31,65,114,0.05)]">상세 보기 ›</button>
                        <button data-design="today-diary-button" type="button" onClick={() => void onSelect?.(entry)} className="today-diary-button inline-flex h-[40px] min-w-0 items-center justify-center gap-2 whitespace-nowrap rounded-full bg-[#2f7eea] px-4 text-[12px] font-extrabold leading-[16px] text-white shadow-[0_8px_18px_rgba(47,126,234,0.26)]"><span className="today-diary-button-icon flex h-[16px] w-[16px] shrink-0 items-center justify-center leading-none"><PencilIcon className="block size-[15px]" /></span><span className="block leading-[16px] whitespace-nowrap">다이어리 쓰기</span></button>
                      </div>
                    </div>
                  </div>
                </div>
