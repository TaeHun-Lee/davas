'use client';

import Link from 'next/link';
import { useRouter } from 'next/navigation';
import { useCallback, useEffect, useRef, useState } from 'react';
import { listRecords, type RecordCardData } from '../../lib/api/core';
import { getFriends } from '../../lib/api/friends';
import { AsyncState, CoreAppShell, EmptyState, RecordCard, SearchField } from './CoreUi';
import {
  recordListFailureMode,
  recordListRequestIsCurrent,
  recordListReturnTo,
  type RecordListFilters,
} from './record-list-model';

function useRecords(scope: 'friends' | 'mine', filters: RecordListFilters) {
  const { q, mediaType, viewingMethod } = filters;
  const requestKey = JSON.stringify([scope, q ?? '', mediaType ?? '', viewingMethod ?? '']);
  const generationRef = useRef({ key: requestKey, value: 0 });
  if (generationRef.current.key !== requestKey) {
    generationRef.current = { key: requestKey, value: generationRef.current.value + 1 };
  }
  const moreRequestRef = useRef(false);
  const [items, setItems] = useState<RecordCardData[]>([]);
  const [cursor, setCursor] = useState<string | null>(null);
  const [hasMore, setHasMore] = useState(false);
  const [status, setStatus] = useState<'loading' | 'ready' | 'error'>('loading');
  const [key, setKey] = useState(0);
  const [moreBusy, setMoreBusy] = useState(false);
  const [moreError, setMoreError] = useState(false);

  const load = useCallback(
    async (next: string | undefined, requestGeneration: number) => {
      const isPagination = Boolean(next);
      if (isPagination && moreRequestRef.current) return;
      if (isPagination) {
        moreRequestRef.current = true;
        setMoreBusy(true);
        setMoreError(false);
      } else {
        moreRequestRef.current = false;
        setMoreBusy(false);
        setMoreError(false);
        setStatus('loading');
      }
      try {
        const page = await listRecords(scope, {
          q,
          mediaType,
          viewingMethod,
          cursor: next,
          limit: 20,
        });
        if (!recordListRequestIsCurrent(requestGeneration, generationRef.current.value)) return;
        setItems((old) => (isPagination ? [...old, ...page.items] : page.items));
        setCursor(page.nextCursor);
        setHasMore(page.hasMore);
        setStatus('ready');
      } catch {
        if (!recordListRequestIsCurrent(requestGeneration, generationRef.current.value)) return;
        if (recordListFailureMode(isPagination) === 'pagination') setMoreError(true);
        else setStatus('error');
      } finally {
        if (recordListRequestIsCurrent(requestGeneration, generationRef.current.value)) {
          moreRequestRef.current = false;
          setMoreBusy(false);
        }
      }
    },
    [scope, q, mediaType, viewingMethod],
  );

  useEffect(() => {
    void load(undefined, generationRef.current.value);
  }, [load, key]);

  return {
    items,
    cursor,
    hasMore,
    status,
    moreBusy,
    moreError,
    retry: () => {
      generationRef.current = {
        ...generationRef.current,
        value: generationRef.current.value + 1,
      };
      setKey((value) => value + 1);
    },
    loadMore: () => {
      if (cursor) void load(cursor, generationRef.current.value);
    },
  };
}

export function RecordList({
  scope,
  filters = {},
}: {
  scope: 'friends' | 'mine';
  filters?: RecordListFilters;
}) {
  const data = useRecords(scope, filters);
  const [hasFriends, setHasFriends] = useState<boolean | null>(null);
  useEffect(() => {
    if (scope === 'friends') {
      void getFriends()
        .then((value) => setHasFriends(value.friends.length > 0))
        .catch(() => setHasFriends(null));
    }
  }, [scope]);

  if (data.status === 'loading') return <AsyncState kind="loading" />;
  if (data.status === 'error') return <AsyncState kind="error" onRetry={data.retry} />;
  if (!data.items.length) {
    const filtered = Boolean(filters.q || filters.mediaType || filters.viewingMethod);
    const noFriends = scope === 'friends' && hasFriends === false;
    return (
      <EmptyState
        title={
          filtered
            ? '조건에 맞는 기록이 없어요'
            : noFriends
              ? '아직 연결된 친구가 없어요.'
              : scope === 'friends'
                ? '아직 공유된 기록이 없어요.'
                : '아직 남긴 기록이 없어요.'
        }
        description={
          filtered
            ? '검색어나 필터를 바꿔 다시 찾아보세요.'
            : noFriends
              ? '친구를 초대하면 서로의 영화·드라마 기록을 볼 수 있어요.'
              : scope === 'friends'
                ? '친구들은 아직 기록을 공유하지 않았어요. 내 첫 기록을 남겨보세요.'
                : '본 영화나 드라마를 첫 기록으로 남겨보세요.'
        }
        action={
          <Link className="primary-button" href={noFriends ? '/friends' : '/records/new'}>
            {noFriends ? '친구 초대하기' : '본 작품 기록하기'}
          </Link>
        }
      />
    );
  }

  const returnTo = recordListReturnTo(scope, filters);
  return (
    <>
      <div className="space-y-4">
        {data.items.map((item) => (
          <RecordCard
            key={item.id}
            item={item}
            variant={scope === 'mine' ? 'mine' : 'feed'}
            returnTo={returnTo}
          />
        ))}
      </div>
      {data.moreError ? (
        <div className="mt-5 text-center" role="alert">
          <p className="form-error">더 보기를 완료하지 못했어요. 기존 기록은 그대로 유지했어요.</p>
          <button className="secondary-button mt-3 w-full" onClick={data.loadMore}>
            더 보기 다시 시도
          </button>
        </div>
      ) : data.hasMore ? (
        <button
          className="secondary-button mt-5 w-full"
          disabled={data.moreBusy}
          onClick={data.loadMore}
        >
          {data.moreBusy ? '불러오는 중…' : '더 보기'}
        </button>
      ) : null}
    </>
  );
}

function SearchLaunchField({
  scope,
  label,
  placeholder,
}: {
  scope: 'friends' | 'mine';
  label: string;
  placeholder: string;
}) {
  const router = useRouter();
  const [value, setValue] = useState('');
  const submit = () => {
    const params = new URLSearchParams({ scope });
    if (value.trim()) params.set('q', value.trim());
    router.push(`/search?${params.toString()}`);
  };
  return (
    <SearchField
      value={value}
      onChange={setValue}
      onSubmit={submit}
      label={label}
      placeholder={placeholder}
    />
  );
}

export function FeedScreen() {
  return (
    <CoreAppShell>
      <h1 className="page-title">친구 기록</h1>
      <p className="page-description">친구들과 최근 본 작품과 감상을 나눠보세요.</p>
      <div className="mt-5">
        <SearchLaunchField
          scope="friends"
          label="친구 기록 검색"
          placeholder="작품이나 친구 이름으로 기록 찾기"
        />
      </div>
      <Link href="/records/new" className="wide-cta mt-4">
        <span>＋ 본 작품 기록하기</span>
        <span aria-hidden="true">›</span>
      </Link>
      <h2 className="section-title mb-3 mt-7">최근 공유된 기록</h2>
      <RecordList scope="friends" />
    </CoreAppShell>
  );
}

export function MineScreen() {
  return (
    <CoreAppShell>
      <h1 className="page-title">내 기록</h1>
      <p className="page-description">공개 여부와 관계없이 내가 본 작품을 모아봐요.</p>
      <div className="mt-5">
        <SearchLaunchField scope="mine" label="내 기록 검색" placeholder="내 기록 검색" />
      </div>
      <Link href="/records/new" className="secondary-button mt-4 w-full">
        ＋ 새 기록 남기기
      </Link>
      <h2 className="section-title mb-3 mt-7">최근 본 작품</h2>
      <RecordList scope="mine" />
    </CoreAppShell>
  );
}
