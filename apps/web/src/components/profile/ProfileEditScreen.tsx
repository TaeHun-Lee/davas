...
export function ProfileEditScreen() {
  ...
  if (!user || status === 'loading')
    return <MediaDetailLoadingIndicator label="프로필 편집 화면을 불러오는 중" />;
  ...
  return (
    <AppShell>
      <form
        ...
        data-design="profile-edit-screen"
      >
        <header className="flex h-[48px] items-center justify-between">
          <button
            type="button"
            onClick={() => router.back()}
            className="text-[14px] font-extrabold text-[#6f7c91]"
          >
            취소
          </button>
          ...
          <button
            ...
            className="text-[14px] font-extrabold text-[#2f65b8]"
          >
            저장
          </button>
        </header>
        <div className="mt-4 rounded-[24px] bg-white p-5 shadow-[0_12px_28px_rgba(31,65,114,0.08)]">
          <ProfileImagePicker
            ...
            displayName={nickname || user.nickname}
          ...
          <label className="mt-6 block text-[13px] font-black text-[#284778]">
            닉네임
            <input
              value={nickname}
              onChange={(event) => setNickname(event.target.value)}
              className="mt-2 h-[46px] w-full rounded-[16px] border border-[#dfe6f0] bg-[#f8fafd] px-4 text-[15px] font-bold outline-none"
            />
          </label>
          <label className="mt-4 block text-[13px] font-black text-[#284778]">
            소개
            <textarea
              value={bio}
              onChange={(event) => setBio(event.target.value)}
              rows={4}
              className="mt-2 w-full resize-none rounded-[16px] border border-[#dfe6f0] bg-[#f8fafd] p-4 text-[14px] font-semibold outline-none"
            />
          </label>
          <div className="mt-4">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setPreferredGenres((items) =>
                        selected ? items.filter((item) => item !== genre) : [...items, genre],
                      )
                    }
                    className={`rounded-full px-3 py-2 text-[12px] font-extrabold ${selected ? 'bg-[#2f65b8] text-white' : 'bg-[#eef3fb] text-[#6f7c91]'}`}
                  >
                    {genre}
                  </button>
                );
          ...
          {status === 'error' ? (
            <p className="mt-4 text-[13px] font-bold text-[#d94f4f]">
              저장하지 못했어요. 다시 시도해주세요.
            </p>
          ) : null}
        </div>
