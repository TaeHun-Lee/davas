...
function fingerprint(dto: CreateDiaryDto) {
  return createHash('sha256')
    .update(JSON.stringify(normalizedCreate(dto)))
    .digest('hex');
}
...
export class DiariesService {
  ...
  async create(userId: string, dto: CreateDiaryDto) {
    ...
    const replay = await this.diaries.findOne({
    ...
    try {
      ...
      await this.syncAuthorProjection(saved, true);
      saved.media = media;
      return { diary: this.toDetail(saved, userId), deduplicated: false };
    } catch (error) {
      if ((error as { code?: string }).code === '23505') {
        const raced = await this.diaries.findOne({
          where: { userId, clientRequestId: dto.clientRequestId! },
          withDeleted: true,
          relations: { media: true, user: true },
        });
  ...
  private resolveReplay(diary: DiaryEntity, expected: string, userId: string) {
    if (diary.clientRequestFingerprint !== expected)
      throw apiError(
        409,
        'IDEMPOTENCY_CONFLICT',
        '같은 요청 식별자로 다른 내용을 저장할 수 없어요.',
      );
    ...
    if (this.watchParticipants) {
      const participant =
        (await this.watchParticipants.findOne({
          where: { diaryId: diary.id, accountId: diary.userId },
        })) ??
        this.watchParticipants.create({
          diaryId: diary.id,
          accountId: diary.userId,
          requestedAt: diary.createdAt ?? new Date(),
        });
      participant.status = 'CONFIRMED';
      participant.respondedAt ??= diary.createdAt ?? new Date();
      await this.watchParticipants.save(participant);
    }
    if (syncReaction && this.watchReactions) {
      const reaction =
        (await this.watchReactions.findOne({
          where: { diaryId: diary.id, accountId: diary.userId },
        })) ??
        this.watchReactions.create({
          diaryId: diary.id,
          accountId: diary.userId,
        });
