...
type LegacyCreateDiaryDto = {
  ...
  mood?: string;
  memoryNote?: string;
  companions?: Array<{ userId?: string; displayName: string }>;
...
export class DiariesDashboardService {
  constructor(
    ...
    @Optional()
    @InjectRepository(DiaryCompanionEntity)
    private readonly companionRepository?: Repository<DiaryCompanionEntity>,
    @Optional()
    @InjectRepository(DiaryShareEntity)
    private readonly shareRepository?: Repository<DiaryShareEntity>,
    @Optional()
  ...
  async createDiary(userId: string, dto: LegacyCreateDiaryDto) {
    ...
    if (
      ...
      return connection.transaction(async (manager) => {
        ...
        const saved = await diaryRepository.save(
          this.createDiaryEntity(diaryRepository, userId, dto),
        );
        await this.replaceRelatedRows(
          saved.id,
          relatedDto,
          manager.getRepository(DiaryCompanionEntity),
          manager.getRepository(DiaryShareEntity),
        );
        const watchlistRepository = manager.getRepository(WatchlistItemEntity);
        const watchlist = await watchlistRepository.findOne({
          where: { userId, mediaId: saved.mediaId },
        });
        if (watchlist) {
  ...
  private createDiaryEntity(
    repository: Repository<DiaryEntity>,
    userId: string,
    dto: LegacyCreateDiaryDto,
  ) {
  ...
  async getDiary(userId: string, id: string) {
    ...
    if (!this.access && (!diary || diary.userId !== userId))
      throw new NotFoundException('다이어리를 찾을 수 없습니다.');
  ...
  async removeDiary(userId: string, id: string) {
    const repository = this.diaries as Repository<DiaryEntity> & {
      findOne: Repository<DiaryEntity>['findOne'];
      softDelete: Repository<DiaryEntity>['softDelete'];
    };
  ...
  private async replaceRelatedRows(
    diaryId: string,
    dto: {
      companions?: LegacyCreateDiaryDto['companions'];
      selectedUserIds?: LegacyCreateDiaryDto['selectedUserIds'];
    },
    companionRepository = this.companionRepository,
    ...
    if (dto.companions !== undefined && companionRepository) {
      ...
      if (companions.length)
        await companionRepository.save(
          companions.map((item) => companionRepository.create({ diaryId, ...item })),
        );
    }
    if (dto.selectedUserIds !== undefined && shareRepository) {
      ...
      if (ids.length)
        await shareRepository.save(
          ids.map((userId) => shareRepository.create({ diaryId, userId })),
        );
    }
  }
  ...
  private async saveRepresentativePoster(dto: {
    mediaId?: string;
    mediaPosterUrl?: string | null;
  }) {
  ...
  async getDashboard(
    ...
    const now = new Date();
    const latestDiaryDate = diaries[0]?.watchedDate
      ? toDateParts(diaries[0].watchedDate)
      : undefined;
    const baseDate = {
    ...
    const averageRating =
      ratedDiaries.length > 0
        ? ratedDiaries.reduce((sum, diary) => sum + Number(diary.rating), 0) / ratedDiaries.length
        : 0;
    ...
    return {
      summary: {
        ...
        topGenre: genreRatios[0]
          ? { name: genreRatios[0].genre, count: genreRatios[0].count }
          : null,
      },
      calendar: {
        year: baseDate.year,
